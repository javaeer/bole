    ...
    <scroll-view class="content-container" scroll-y>
        ...
        <template v-if="!isEditMode">
          ...
          <view class="info-group">
            ...
            <view class="info-item">
              ...
              <view class="info-value link">{{ companyDetail?.website }}</view>
            </view>
          </view>
        </template>
      ...
      <view class="section-container">
        <view class="section-header">
          <view class="section-title">评价 ({{ commentList.length }})</view>
          <view
    ...
    <view v-if="!isEditMode" class="action-bar">
      ...
      >
        <uni-icons
          ...
          size="20"
          :color="isFollowed ? 'white' : '$primary-color'"
        />
...
// 评论相关
const commentList = ref<CompanyComment[]>([]);
...
const loadComments = async (companyId: number) => {
  try {
    commentList.value = await CompanyCommentAPI.page(companyId);
  } catch (error) {
...
const submitComment = async () => {
  ...
  try {
    const comment = await companyService.addComment(
      companyDetail.value.id,
...
const deleteComment = async (commentId: number) => {
  try {
    await companyService.deleteComment(commentId);
