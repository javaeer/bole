...
public class YunlouAuthenticationEntryPoint implements AuthenticationEntryPoint {
    ...
    public void commence(HttpServletRequest request,
        ...
        log.error("未授权访问尝试: {} {} - 原因: {}", request.getMethod(), request.getRequestURI(), authException.getMessage());
        response.setStatus(HttpServletResponse.SC_UNAUTHORIZED);
        response.setCharacterEncoding("UTF-8");
        response.setContentType(MediaType.APPLICATION_JSON_VALUE);

        String body = JsonUtils.toJson(BusinessResponse.error(BusinessStatus.UNAUTHORIZED_INVALID_EXPIRED));
