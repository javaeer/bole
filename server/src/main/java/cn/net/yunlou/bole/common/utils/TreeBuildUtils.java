 ...
 *
 *         // 构建树形结构
 *         return TreeBuildUtils.buildFullTree(allDepartments);
 *     }
 *
 *     public List<Department> getDepartmentTreeByParent(Long parentId) {
 *         List<Department> allDepartments = departmentMapper.selectList(null);
 *
 *         // 按sort字段排序
 *         return TreeBuildUtils.buildTree(allDepartments, parentId,
 *             Comparator.comparing(Department::getSort));
 *     }
 *
 *     public List<Department> getDepartmentTreeCustomSort() {
 *         List<Department> allDepartments = departmentMapper.selectList(null);
 *
 *         // 自定义排序：先按sort，再按name
 *         Comparator<Department> comparator = Comparator
 *             .comparing(Department::getSort)
 *             .thenComparing(Department::getName);
 *
 *         return TreeBuildUtils.buildTree(allDepartments, BaseTreeEntity.ROOT_ID, comparator);
 *     }
 */
public class TreeBuildUtils {
    ...
    public static <T extends BaseTreeEntity<T>> List<T> buildTree(List<T> allNodes, Long rootId, 
        ...
        // 按父ID分组
        Map<Long, List<T>> childrenMap = allNodes.stream()
     ...
     * 递归检测循环引用
     */
    private static <T extends BaseTreeEntity<T>> boolean hasCircularReference(T node, 
                                                                             Map<Long, T> nodeMap, 
                                                                             Set<Long> path) {
        ...
        // 检查父节点链
        Long parentId = node.getParentId();
        if (parentId != null && !parentId.equals(BaseTreeEntity.ROOT_ID) && nodeMap.containsKey(parentId)) {
            T parent = nodeMap.get(parentId);
