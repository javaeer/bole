...
/**
 * 自定义 Wrappers 工具类，提供忽略无效值的包装器 简化包装器的创建，提供统一的入口
 *
 * <p>// 基础使用 SkipInvalidValueLambdaQueryWrapper<User> queryWrapper =
 * SkipInvalidValueWrappers.lambdaQuery(user);
 *
 * <p>SkipInvalidValueLambdaUpdateWrapper<User> updateWrapper =
 * SkipInvalidValueWrappers.lambdaUpdate(user);
 *
 * <p>// 严格模式（调试用） SkipInvalidValueLambdaQueryWrapper<User> strictQueryWrapper =
 * SkipInvalidValueWrappers.strictLambdaQuery(user);
 *
 * <p>// 允许null值 SkipInvalidValueLambdaUpdateWrapper<User> lenientUpdateWrapper =
 * SkipInvalidValueWrappers.lenientLambdaUpdate(user);
 *
 * <p>// 空构造 SkipInvalidValueLambdaQueryWrapper<User> emptyQueryWrapper =
 * SkipInvalidValueWrappers.lambdaQuery();
 *
 * <p>// 基于类名构造 SkipInvalidValueLambdaQueryWrapper<User> classQueryWrapper =
 * SkipInvalidValueWrappers.lambdaQuery(User.class);
 *
 * <p>// 组合使用 SkipInvalidValueLambdaQueryWrapper<User> combinedWrapper =
...
public class SkipInvalidValueWrappers {
    ...
    public static <T> SkipInvalidValueLambdaQueryWrapper<T> strictLambdaQuery(
            Class<T> entityClass) {
    ...
    public static <T> SkipInvalidValueLambdaUpdateWrapper<T> strictLambdaUpdate(
            Class<T> entityClass) {
    ...
    public static <T> SkipInvalidValueLambdaQueryWrapper<T> strictAndLenientLambdaQuery(T entity) {
        return SkipInvalidValueLambdaQueryWrapper.of(entity).strictMode().allowNullValue();
    }

    /** 获取严格模式且允许Null值的 LambdaUpdateWrapper */
    public static <T> SkipInvalidValueLambdaUpdateWrapper<T> strictAndLenientLambdaUpdate(
            T entity) {
        return SkipInvalidValueLambdaUpdateWrapper.of(entity).strictMode().allowNullValue();
    }
}
