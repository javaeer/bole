...
public class YunlouAccessDeniedHandler implements AccessDeniedHandler {
    ...
    public void handle(HttpServletRequest request,
        ...
        String username = SecurityContextUtils.getCurrentUsername() != null
                ? SecurityContextUtils.getCurrentUsername()
                : "unknown";

        log.warn("权限拒绝: 用户[{}]尝试访问 {} {} - 权限不足", username, request.getMethod(), request.getRequestURI());

        response.setStatus(HttpServletResponse.SC_FORBIDDEN);
        response.setContentType(MediaType.APPLICATION_JSON_VALUE);
        response.setCharacterEncoding("UTF-8");

        String body = JsonUtils.toJson(BusinessResponse.error(BusinessStatus.FORBIDDEN_NO_PERMISSION));
