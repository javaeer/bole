...
public interface IEnumCode<T, C> extends IEnum<T>, Serializable {
     ...
     * @return 枚举标签
     */
    static <E extends Enum<E> & IEnumCode<T, C>, T, C> String codeOfLabel(C code, Class<E> enumType) {
        E enumInstance = codeOf(code, enumType);
    ...
    static <E extends Enum<E> & IEnumCode<T, C>, T, C> List<EnumCodeItem<T, C>> toEnumCodeItemList(Class<E> enumType) {
        Objects.requireNonNull(enumType, "EnumType cannot be null");
        return new ArrayList<>((List<EnumCodeItem<T, C>>) (List<?>)
                EnumCodeCacheManager.getFullInfoCache(enumType));
    }
     ...
     * @return 映射Map
     */
    static <E extends Enum<E> & IEnumCode<T, C>, T, C> Map<C, IEnum.EnumItem<T>> toEnumCodeItemMap(Class<E> enumType) {
        Objects.requireNonNull(enumType, "EnumType cannot be null");
        return EnumSet.allOf(enumType).stream()
    ...
    class EnumCodeCacheManager {
        ...
        @SuppressWarnings("unchecked")
        static <E extends Enum<E> & IEnumCode<?, ?>> Map<Object, E> getValueCache(Class<E> enumType) {
        ...
        @SuppressWarnings("unchecked")
        static <E extends Enum<E> & IEnumCode<?, ?>> Map<Object, E> getCodeCache(Class<E> enumType) {
            return (Map<Object, E>) CODE_CACHE.computeIfAbsent(enumType, k ->
                    EnumSet.allOf(enumType).stream()
                            .collect(Collectors.toMap(
        ...
        @SuppressWarnings("unchecked")
        static <E extends Enum<E> & IEnumCode<?, ?>> List<EnumCodeItem<?, ?>> getFullInfoCache(Class<E> enumType) {
            return (List<EnumCodeItem<?, ?>>) FULL_INFO_CACHE.computeIfAbsent(enumType, k ->
                    EnumSet.allOf(enumType).stream()
                            .map(e -> {
                                IEnumCode<?, ?> ie = (IEnumCode<?, ?>) e;
                                return new EnumCodeItem<>(ie.getValue(), ie.getLabel(), ie.getCode());
                            })
                            .collect(Collectors.toList())
            );
        }
