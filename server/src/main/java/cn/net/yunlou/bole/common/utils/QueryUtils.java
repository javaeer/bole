...
/**
 * 模型工具类 - 专注于BaseQuery及其子类的特殊处理
 *
 * <p>职责范围： ✅ BaseQuery特定格式转换 ✅ 下划线到驼峰命名字段名转换 ✅ 嵌套BaseQuery递归处理 ✅ 与BeanUtils和ReflectUtils整合
 *
 * @author javaeer
 * @since 2019/9/10
 */
...
public class QueryUtils {
    ...
    /**
     * 将BaseQuery转换为Map（支持下划线到驼峰命名转换）
     *
     * <p>只支持继承自BaseQuery的对象
     *
     * @param model BaseQuery实例
     * @param <M> 模型类型
     * @return 转换后的Map
     */
    ...
    public static <M> Map<String, Object> modelToMap(M model, boolean convertFieldName) {
        ...
        try {
            if (!(model instanceof BaseQuery)) {
                log.warn("Model is null or not instance of BaseQuery, returning empty map");
                return map;
            }
            ...
            log.debug(
                    "Successfully converted BaseQuery to map: {}",
                    model.getClass().getSimpleName());
            return map;

        } catch (Exception e) {
            log.error(
                    "Failed to convert BaseQuery to map: {}", model.getClass().getSimpleName(), e);
    ...
    private static void processField(
            Object model, Field field, Map<String, Object> map, boolean convertFieldName) {
        try {
            // 跳过不需要处理的字段
            ...
            log.warn(
                    "Failed to process field {} in model {}, skipping",
                    field.getName(),
                    model.getClass().getSimpleName(),
                    e);
        }
    ...
    private static Object processFieldValue(Object fieldValue, Class<?> fieldType) {
        ...
        // BaseQuery递归处理
        if (fieldValue instanceof BaseQuery) {
    ...
    private static Object processSingleElement(Object element) {
        ...
        if (element instanceof BaseQuery) {
    ...
    private static Object tryConvertToMap(Object obj) {
        try {
            ...
            log.debug(
                    "Failed to convert object to map, using original value: {}",
    ...
    private static boolean isSimpleType(Class<?> clazz) {
        return SIMPLE_TYPES.contains(clazz) || clazz.isEnum() || isPrimitiveWrapper(clazz);
    }
    ...
    /**
     * 将BaseQuery转换为指定类型的Bean
     *
     * @param om BaseQuery实例
     * @param clazz 目标Bean类型
     * @param <T> 目标类型
     * @return 转换后的Bean
     */
    public static <T> T modelToBean(BaseQuery om, Class<T> clazz) {
     ...
     * @return 转换后的Bean
     */
    public static <T> T modelToBean(BaseQuery om, Class<T> clazz, boolean convertFieldName) {
        if (om == null) {
            return null;
        }
        ...
        try {
            ...
            log.error("Failed to convert BaseQuery to bean: {}", clazz.getSimpleName(), e);
            throw new ModelConversionException("Model to bean conversion failed", e);
        }
    }

    /**
     * 批量将BaseQuery列表转换为Bean列表
     *
     * @param models BaseQuery列表
     * @param clazz 目标Bean类型
     * @param <T> 目标类型
     * @return 转换后的Bean列表
     */
    public static <T> List<T> modelToBeans(List<? extends BaseQuery> models, Class<T> clazz) {
     ...
     * @return 转换后的Bean列表
     */
    public static <T> List<T> modelToBeans(
            List<? extends BaseQuery> models, Class<T> clazz, boolean convertFieldName) {
     ...
     *
     * @param bean 源Bean
     * @param modelClass 目标BaseQuery类型
     * @param <T> BaseQuery类型
     * @return 转换后的BaseQuery
     */
    public static <T extends BaseQuery> T beanToModel(Object bean, Class<T> modelClass) {
     ...
     * @return 转换后的BaseQuery
     */
    public static <T extends BaseQuery> T beanToModel(
            Object bean, Class<T> modelClass, boolean convertFieldName) {
        if (bean == null) {
            return null;
        }
        ...
        try {
            ...
            log.error("Failed to convert bean to BaseQuery: {}", modelClass.getSimpleName(), e);
            throw new ModelConversionException("Bean to model conversion failed", e);
        }
    }

    /** 转换Map中的字段名 */
    private static Map<String, Object> convertFieldNames(
            Map<String, Object> map, boolean convertFieldName) {
        if (!convertFieldName) {
            return map;
        }
    ...
    /** 判断对象是否为BaseQuery */
    public static boolean isBaseQuery(Object obj) {
        return obj instanceof BaseQuery;
    }

    /** 获取BaseQuery的类信息 */
    public static Class<?> getBaseQueryClass(Object obj) {
        return isBaseQuery(obj) ? obj.getClass() : null;
    }
