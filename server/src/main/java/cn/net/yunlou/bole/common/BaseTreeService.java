...
/**
 * @Author javaeer(javaeer @ aliyun.com) @Date 2019/12/12 09:20 @Version 1.0
 */
...
public abstract class BaseTreeService<
    ...
    public boolean updateById(T entity) {
        ...
        if (result) {
            ...
            if (oldEntity != null
                    && !Objects.equals(oldEntity.getParentId(), entity.getParentId())) {
    ...
    public boolean removeById(Serializable id) {
        ...
        if (!Collections.isEmpty(allChildren)) {
            List<Serializable> childIds =
                    allChildren.stream().map(T::getId).collect(Collectors.toList());
    ...
    public List<T> listChildren(Serializable id) {
        ...
        queryWrapper.eq("parent_id", id).orderByAsc("id");
    ...
    /**
     * desc: 遍历整棵树结构 子节点对象集合，将子所有节点追加到 children 完整的 全部的 所有的
     *
     * @return {@link null}
     * @author javaeer(javaeer @ aliyun.com)
    ...
    /**
     * desc: 获取指定节点的<下级>对象的集合，将子所有节点追加到<所属节点>的children
     *
     * @param id
     * @return {@link null}
     * @author javaeer(javaeer @ aliyun.com)
    ...
    public List<T> listAllChildren(Serializable id) {
        ...
        QueryWrapper<T> queryWrapper = new QueryWrapper<>();
        queryWrapper.likeRight("path", parent.getPath() + "/").orderByAsc("level", "id");
    ...
    public List<T> listAllChildren(T entity) {
        ...
        if (!Collections.isEmpty(children)) {
            for (T child : children) {
                // 使用path字段进行like查询，一次获取所有子孙节点
                QueryWrapper<T> queryWrapper = new QueryWrapper<>();
                queryWrapper.likeRight("path", child.getPath() + "/").orderByAsc("level", "id");
