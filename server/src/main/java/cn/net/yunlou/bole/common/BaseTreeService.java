...
public abstract class BaseTreeService<
        ...
        try {
            if (lock.tryLock(5, 10, TimeUnit.SECONDS)) {
                ...
                if (result) {
                    ...
                    log.debug("保存树节点成功，节点ID: {}, 路径: {}",
                            entity.getId(), entity.getPath());
                }
                return result;
            } else {
    ...
    public boolean saveBatch(Collection<T> entityList) {
        ...
        // 按父节点分组，同一父节点的批量操作
        Map<Long, List<T>> groupByParent = entityList.stream()
    ...
    @Transactional(rollbackFor = Exception.class)
    @Caching(evict = {
            @CacheEvict(cacheResolver = "treeCacheResolver", key = "#root.target.buildCacheKey('with_children:' + #entity.id)"),
            @CacheEvict(cacheResolver = "treeCacheResolver", key = "#root.target.buildCacheKey('with_tree:' + #entity.id)"),
            @CacheEvict(cacheResolver = "treeCacheResolver", key = "#root.target.buildCacheKey('children:' + #entity.id)"),
            @CacheEvict(cacheResolver = "treeCacheResolver", key = "#root.target.buildCacheKey('all_children:' + #entity.id)"),
            @CacheEvict(cacheResolver = "treeCacheResolver", key = "#root.target.buildCacheKey('leaves:' + #entity.id)"),
            @CacheEvict(cacheResolver = "treeCacheResolver", key = "#root.target.buildCacheKey('parent:' + #entity.id)"),
            @CacheEvict(cacheResolver = "treeCacheResolver", key = "#root.target.buildCacheKey('grandpa:' + #entity.id)"),
            @CacheEvict(cacheResolver = "treeCacheResolver", key = "#root.target.buildCacheKey('root:' + #entity.id)"),
            @CacheEvict(cacheResolver = "treeCacheResolver", key = "#root.target.buildCacheKey('children:' + #entity.parentId)"),
            @CacheEvict(cacheResolver = "treeCacheResolver", key = "#root.target.buildCacheKey('all_children:' + #entity.parentId)")
    })
    ...
    @Transactional(rollbackFor = Exception.class)
    @Caching(evict = {
            @CacheEvict(cacheResolver = "treeCacheResolver", key = "#root.target.buildCacheKey('with_children:' + #id)"),
            @CacheEvict(cacheResolver = "treeCacheResolver", key = "#root.target.buildCacheKey('with_tree:' + #id)"),
            @CacheEvict(cacheResolver = "treeCacheResolver", key = "#root.target.buildCacheKey('children:' + #id)"),
            @CacheEvict(cacheResolver = "treeCacheResolver", key = "#root.target.buildCacheKey('all_children:' + #id)"),
            @CacheEvict(cacheResolver = "treeCacheResolver", key = "#root.target.buildCacheKey('leaves:' + #id)"),
            @CacheEvict(cacheResolver = "treeCacheResolver", key = "#root.target.buildCacheKey('parent:' + #id)"),
            @CacheEvict(cacheResolver = "treeCacheResolver", key = "#root.target.buildCacheKey('grandpa:' + #id)"),
            @CacheEvict(cacheResolver = "treeCacheResolver", key = "#root.target.buildCacheKey('root:' + #id)")
    })
    ...
    public boolean removeByIds(Collection<?> list) {
        ...
        // 检查是否有节点存在子节点
        List<Long> nodesWithChildren = ids.stream()
                .filter(this::hasChildNodes)
                .collect(Collectors.toList());
    ...
    public boolean removeNodesByPath(String path) {
        ...
        try {
            if (lock.tryLock(5, 30, TimeUnit.SECONDS)) {
                ...
                queryWrapper.eq("path", path)  // 当前节点
                        .or()
    ...
    public List<T> listLeafNodes(Serializable id) {
        ...
        QueryWrapper<T> queryWrapper = getBaseQueryWrapper(null);
        queryWrapper.likeRight("path", parent.getPath() + "/")
                .apply("NOT EXISTS (SELECT 1 FROM {0} t2 WHERE t2.parent_id = {0}.id)", getTableName());
     ...
     */
    @Cacheable(cacheResolver = "treeCacheResolver",
            key = "#root.target.buildCacheKey('children_page:' + #id + ':' + #page.current + ':' + #page.size)",
            unless = "!#root.target.isCacheEnabled()")
    ...
    @Override
    @Cacheable(cacheResolver = "treeCacheResolver",
            key = "#root.target.buildCacheKey('subtree_page:' + #id + ':' + #depth + ':' + #page.current + ':' + #page.size)",
            unless = "!#root.target.isCacheEnabled()")
    ...
    public boolean moveNodesToParent(List<Long> nodeIds, Long targetParentId) {
        ...
        try {
            if (lock.tryLock(10, 120, TimeUnit.SECONDS)) {
                ...
                // 批量更新
                List<T> updatedNodes = nodesToMove.stream()
                        .map(node -> {
                            T updatedNode = structMapper.copy(node);
                            updatedNode.setParentId(targetParentId);
                            // 更新路径
                            String newPath = targetParent != null ?
                                    targetParent.getPath() + "/" + node.getId() :
                                    "/" + node.getId();
                            updatedNode.setPath(newPath);
                            updatedNode.setLevel(targetParent != null ?
                                    targetParent.getLevel() + 1 : 1);
                            return updatedNode;
                        })
                        .collect(Collectors.toList());
                ...
                updatedNodes.forEach(node -> {
                    String oldPath = nodesToMove.stream()
    ...
    private void updateDescendantsPath(T parentNode, String oldParentPath) {
        ...
        if (getBaseMapper() instanceof IBaseTreeMapper) {
            ...
            int updated = treeMapper.updateChildrenPath(
                    getTableName(),
                    oldParentPath,
                    parentNode.getPath()
            );
            log.debug("更新子节点路径完成，父节点ID: {}, 更新节点数: {}",
